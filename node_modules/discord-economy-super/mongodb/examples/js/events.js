...
eco.on('balanceSet', balance => {
    console.log(`Someone's just set ${balance.amount} coins for balance for member ${balance.memberID} on guild ${balance.guildID}. their balance is ${balance.balance} coins now.\nReason: ${balance.reason}\nOperation type: '${balance.type}'`)
})
eco.on('balanceAdd', balance => {
    console.log(`Someone's just added ${balance.amount} coins for balance for member ${balance.memberID} on guild ${balance.guildID}. their balance is ${balance.balance} coins now.\nReason: ${balance.reason}\nOperation type: '${balance.type}'`)
})
...
bot.on('messageCreate', async message => {
    ...
    if (message.content.startsWith('+shop_buy')) {
        ...
        if (purchase == 'max') return message.channel.send(`You cannot have more than **${item.maxAmount}** of item "**${item.name}**".`)
        return message.channel.send(`You have received item "**${item.name}**" for **${item.price}** coins!`)
    }
    ...
    if (message.content.startsWith('+shop_inventory')) {
        ...
        return message.channel.send(inv.map((x, i) => `ID: ${i + 1}: ${x.name} - ${x.price} coins (${x.date})`).join('\n'))
    }
    ...
    if (message.content.startsWith('+shop_history')) {
        ...
        return message.channel.send(history.map(x => `ID: ${x.id}: ${x.name} - ${x.price} coins (${x.date})`).join('\n'))
    }
